...
#if BASE_DCHECK_IS_ON
#define BASE_CS_DEBUG_CODE(x) x
#else  // !BASE_DCHECK_IS_ON
#define BASE_CS_DEBUG_CODE(x)
#endif  // !BASE_DCHECK_IS_ON
...
#endif
  BASE_CS_DEBUG_CODE(thread_ = 0);
  BASE_CS_DEBUG_CODE(recursion_count_ = 0);
  BASE_UNUSED(thread_);
  BASE_UNUSED(recursion_count_);
#else
...
void RecursiveCriticalSection::Enter() const {
#if defined(__OS_WIN__)
  ...
  if (!have_lock && AtomicOps::Increment(&lock_queue_) > 1) {
    ...
    // have succeeded.
    ABSL_DCHECK(!IsThreadRefEqual(owning_thread_, self));
    // Wait for the lock to become available.
    dispatch_semaphore_wait(semaphore_, DISPATCH_TIME_FOREVER);
    ABSL_DCHECK(owning_thread_ == 0);
    ABSL_DCHECK(!recursion_);
  }
  ...
  if (!recursion_count_) {
    ...
    ABSL_DCHECK(CurrentThreadIsOwner());
  }
  ++recursion_count_;
#endif
#else
...
void RecursiveCriticalSection::Leave() const {
  ABSL_DCHECK(CurrentThreadIsOwner());
#if defined(__OS_WIN__)
...
#if BASE_DCHECK_IS_ON
  --recursion_count_;
  ABSL_DCHECK(recursion_count_ >= 0);
  if (!recursion_count_) {
    thread_ = 0;
  }
#endif
#if defined(__OS_MAC__) && !USE_NATIVE_MUTEX_ON_MAC
  ABSL_DCHECK(IsThreadRefEqual(owning_thread_, CurrentThreadRef()));
  ABSL_DCHECK_GE(recursion_, 0);
  --recursion_;
  if (!recursion_) {
    owning_thread_ = 0;
  }
