...
#ifndef BASE_LINKED_HASH_MAP_H_
#define BASE_LINKED_HASH_MAP_H_
...
//  comments suppress erroneous presubmit failures.
template <class Key,                      //
          class Value,                    //
...
class LinkedHashMap {                     //
 ...
 public:
  ...
  explicit LinkedHashMap(size_type bucket_count) : map_(bucket_count) {}

  LinkedHashMap(const LinkedHashMap& other) = delete;
  LinkedHashMap& operator=(const LinkedHashMap& other) = delete;
  LinkedHashMap(LinkedHashMap&& other) = default;
  LinkedHashMap& operator=(LinkedHashMap&& other) = default;
  ...
  // map and list, a ABSL_CHECK() error will occur.
  iterator erase(iterator position) {
    typename MapType::iterator found = map_.find(position->first);
    ABSL_CHECK(found->second == position)
