...
#if !defined(ABSL_HAVE_THREAD_LOCAL) && defined(__OS_POSIX__)
#include <pthread.h>
#endif
...
void InitializeTls() {
  ABSL_CHECK_EQ(pthread_key_create(&g_current_yield_policy_tls, nullptr), 0);
}

pthread_key_t GetCurrentYieldPolicyTls() {
  static pthread_once_t init_once = PTHREAD_ONCE_INIT;
  ABSL_CHECK_EQ(pthread_once(&init_once, &InitializeTls), 0);
