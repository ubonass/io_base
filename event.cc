...
bool Event::Wait(absl::Duration give_up_after, absl::Duration /*warn_after*/) {
  ScopedYieldPolicy::YieldExecution();
  const DWORD ms = absl::time_internal::IsInfiniteDuration(give_up_after)
                       ? INFINITE
                       : std::max(static_cast<int64_t>(1),
                                  ToInt64Milliseconds(give_up_after));
...
Event::Event(bool manual_reset, bool initially_signaled)
  ...
  ABSL_CHECK(pthread_mutex_init(&event_mutex_, nullptr) == 0);
  pthread_condattr_t cond_attr;
  ABSL_CHECK(pthread_condattr_init(&cond_attr) == 0);
#if USE_CLOCK_GETTIME && !USE_PTHREAD_COND_TIMEDWAIT_MONOTONIC_NP
  ABSL_CHECK(pthread_condattr_setclock(&cond_attr, CLOCK_MONOTONIC) == 0);
#endif
  ABSL_CHECK(pthread_cond_init(&event_cond_, &cond_attr) == 0);
...
timespec GetTimespec(absl::Duration duration_from_now) {
  timespec ts;
  ...
  int64_t microsecs_from_now = absl::ToInt64Microseconds(duration_from_now);
...
bool Event::Wait(absl::Duration give_up_after, absl::Duration warn_after) {
  ...
  // consistent with auto-reset events in _WIN32
